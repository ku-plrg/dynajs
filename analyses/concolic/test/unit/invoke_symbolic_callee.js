...
// TypeError, not leak the engine's value-wrapper ("...apply was called on
// #<Object>"). invokeFunPre peeks the callee, so a wrapped symbolic primitive
...
// AHG harness invoking a symbolic value (e.g. minimist's `flags.unknownFn(arg)`).
{
  ...
  let error = '';
  try {
    callee(); // raw non-callable -> plain TypeError, no wrapper artifact
  } catch (e) {
    error = String(e);
  }
