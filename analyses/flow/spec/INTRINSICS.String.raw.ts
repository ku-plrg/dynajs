...
export function INTRINSICS_String_raw ($ : SpecRuntime, $this : Lifted<unknown>, template : Lifted<unknown>, ...substitutions : Lifted<unknown>[]) {
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 495, $.lessThanEqual(literalCount, $.default<number>(0, [])))))
  {
  ...
  while (true)
  {
    ...
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 496, $.is($.add((nextIndex as Lifted<number>), ($.default<number>(1, []) as Lifted<number>)), literalCount))))
    {
      return R;
    }

    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 497, $.lessThan(nextIndex, substitutionCount))))
    {
