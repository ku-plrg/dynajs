...
export function INTRINSICS_String_prototype_endsWith ($ : SpecRuntime, $this : Lifted<unknown>, searchString : Lifted<unknown>, endPosition : Lifted<unknown> = $.default<undefined>(undefined, [])) {
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 424, $.is(isRegExp, $.default<boolean>(true, [])))))
  {
    throw new TypeError;
  }
  ...
  var len = $.length(S);
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 425, $.is(endPosition, $.default<undefined>(undefined, [])))))
  {
    var pos = len;
  }
  else
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 426, $.is(searchLength, $.default<number>(0, [])))))
  {
    return $.default<boolean>(true, []);
  }

  var start = $.subtract((end as Lifted<number>), (searchLength as Lifted<number>));
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 427, $.lessThan(start, $.default<number>(0, [])))))
  {
    return $.default<boolean>(false, []);
  }

  var substring = $.substring(S, (start as Lifted<number>), (end as Lifted<number>));
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 428, $.is(substring, searchStr))))
  {
    return $.default<boolean>(true, []);
  }
