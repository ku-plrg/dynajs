...
export function AO__CanonicalNumericIndexString ($ : SpecRuntime, argument : Lifted<string>) {
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 10, $.is(argument, $.default<string>("-0", [])))))
  {
    return $.default<number>(0, []);
  }

  var n = AO__ToNumber($, (argument as Lifted<unknown>));
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 11, $.is(AO__ToString($, (n as Lifted<unknown>)), argument))))
  {
    return n;
  }
