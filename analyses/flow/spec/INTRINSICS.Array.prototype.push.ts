...
export function INTRINSICS_Array_prototype_push ($ : SpecRuntime, $this : Wrapped<unknown>, ...items : Wrapped<unknown>[]) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 157, $.greaterThan($.add((len as Wrapped<number>), (argCount as Wrapped<number>)), $.subtract(($.exponentiate($.base<number>(2, []), $.base<number>(53, [])) as Wrapped<number>), ($.base<number>(1, []) as Wrapped<number>)))))
  {
    throw new TypeError;
  }
