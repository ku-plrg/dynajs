...
export function INTRINSICS_Array_prototype_concat ($ : SpecRuntime, $this : Wrapped<unknown>, ...items : Wrapped<unknown>[]) {
  ...
  for (var E of items)
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 110, $.is(spreadable, $.base<boolean>(true, []))))
    {
      var len = AO__LengthOfArrayLike($, (E as Wrapped<unknown>));
      if ($.condition(Number.MAX_SAFE_INTEGER - 111, $.greaterThan($.add((n as Wrapped<number>), (len as Wrapped<number>)), $.subtract(($.exponentiate($.base<number>(2, []), $.base<number>(53, [])) as Wrapped<number>), ($.base<number>(1, []) as Wrapped<number>)))))
      {
        throw new TypeError;
      }
      ...
      while ($.condition(Number.MAX_SAFE_INTEGER - 112, $.lessThan(k, len)))
      {
        ...
        if ($.condition(Number.MAX_SAFE_INTEGER - 113, $.is(exists, $.base<boolean>(true, []))))
        {
    ...
    {
      if ($.condition(Number.MAX_SAFE_INTEGER - 114, $.greaterThanEqual(n, $.subtract(($.exponentiate($.base<number>(2, []), $.base<number>(53, [])) as Wrapped<number>), ($.base<number>(1, []) as Wrapped<number>)))))
      {
        throw new TypeError;
      }
