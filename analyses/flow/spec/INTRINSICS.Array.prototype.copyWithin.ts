...
export function INTRINSICS_Array_prototype_copyWithin ($ : SpecRuntime, $this : Wrapped<unknown>, target : Wrapped<unknown>, start : Wrapped<unknown>, end : Wrapped<unknown> = $.undef) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 105, $.is(relativeTarget, $.base<number>(-Infinity, []))))
  {
    var to = $.base<number>(0, []);
  }
  else
  {
    if ($.condition(Number.MAX_SAFE_INTEGER - 106, $.lessThan(relativeTarget, $.base<number>(0, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 107, $.is(relativeStart, $.base<number>(-Infinity, []))))
  {
    var from = $.base<number>(0, []);
  }
  else
  {
    if ($.condition(Number.MAX_SAFE_INTEGER - 108, $.lessThan(relativeStart, $.base<number>(0, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 109, $.is(end, $.base<undefined>(undefined, []))))
  {
    var relativeEnd = len;
  }
  else
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 110, $.is(relativeEnd, $.base<number>(-Infinity, []))))
  {
    var final = $.base<number>(0, []);
  }
  else
  {
    if ($.condition(Number.MAX_SAFE_INTEGER - 111, $.lessThan(relativeEnd, $.base<number>(0, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 112, $.lessThan(from, to)) && $.condition(Number.MAX_SAFE_INTEGER - 113, $.lessThan(to, $.add((from as Wrapped<number>), (count as Wrapped<number>)))))
  {
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 114, $.greaterThan(count, $.base<number>(0, []))))
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 115, $.is(fromPresent, $.base<boolean>(true, []))))
    {
