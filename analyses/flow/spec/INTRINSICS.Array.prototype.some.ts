...
export function INTRINSICS_Array_prototype_some ($ : SpecRuntime, $this : Lifted<unknown>, callback : Lifted<unknown>, thisArg : Lifted<unknown> = $.default<undefined>(undefined, [])) {
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 212, $.is(AO__IsCallable($, (callback as Lifted<unknown>)), $.default<boolean>(false, [])))))
  {
    throw new TypeError;
  }

  var k = $.default<number>(0, []);
  while ($.value($.condition(Number.MAX_SAFE_INTEGER - 213, $.lessThan(k, len))))
  {
    ...
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 214, $.is(kPresent, $.default<boolean>(true, [])))))
    {
      ...
      if ($.value($.condition(Number.MAX_SAFE_INTEGER - 215, $.is(testResult, $.default<boolean>(true, [])))))
      {
