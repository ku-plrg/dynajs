...
export function INTRINSICS_Array_prototype_reduce ($ : SpecRuntime, $this : Wrapped<unknown>, callback : Wrapped<unknown>, initialValue : Wrapped<unknown> = $.undef) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 167, $.is(AO__IsCallable($, (callback as Wrapped<unknown>)), $.base<boolean>(false, []))))
  {
    throw new TypeError;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 168, $.is(len, $.base<number>(0, []))) && !initialValueIsPresent)
  {
    throw new TypeError;
  }
  ...
  else
  {
    ...
    while ($.condition(Number.MAX_SAFE_INTEGER - 169, $.is(kPresent, $.base<boolean>(false, []))) && $.condition(Number.MAX_SAFE_INTEGER - 170, $.lessThan(k, len)))
    {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 172, $.is(kPresent, $.base<boolean>(false, []))))
    {
      throw new TypeError;
    }
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 173, $.lessThan(k, len)))
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 174, $.is(kPresent, $.base<boolean>(true, []))))
    {
