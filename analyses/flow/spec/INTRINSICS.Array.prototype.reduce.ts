...
export function INTRINSICS_Array_prototype_reduce ($ : SpecRuntime, $this : Wrapped<unknown>, callback : Wrapped<unknown>, initialValue : Wrapped<unknown> = $.undef) {
  ...
  else
  {
    ...
    while ($.condition(Number.MAX_SAFE_INTEGER - 160, $.is(kPresent, $.base<boolean>(false, []))) && $.condition(Number.MAX_SAFE_INTEGER - 161, $.lessThan(k, len)))
    {
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 164, $.lessThan(k, len)))
  {
    var Pk = AO__ToString($, (k as Wrapped<unknown>));
