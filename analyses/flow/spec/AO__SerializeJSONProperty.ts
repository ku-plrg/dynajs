...
export function AO__SerializeJSONProperty ($ : SpecRuntime, state : Wrapped<unknown>, key : Wrapped<string>, holder : Wrapped<unknown>) {
  ...
  if (($.isType(value, "object")) || ($.isType(value, "bigint")))
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 567, $.is(AO__IsCallable($, (toJSON as Wrapped<unknown>)), $.base<boolean>(true, []))))
    {
  ...
  if (!$.condition(Number.MAX_SAFE_INTEGER - 568, $.is(state["ReplacerFunction" /* TODO INTERNAL : internal access */], $.base<undefined>(undefined, []))))
  {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 569, $.is(value, $.base<null>(null, []))))
  {
    return $.base<string>("null", []);
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 570, $.is(value, $.base<boolean>(true, []))))
  {
    return $.base<string>("true", []);
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 571, $.is(value, $.base<boolean>(false, []))))
  {
  ...
  if (($.isType(value, "object")) && $.condition(Number.MAX_SAFE_INTEGER - 572, $.is(AO__IsCallable($, (value as Wrapped<unknown>)), $.base<boolean>(false, []))))
  {
    var isArray = AO__IsArray($, (value as Wrapped<unknown>));
    if ($.condition(Number.MAX_SAFE_INTEGER - 573, $.is(isArray, $.base<boolean>(true, []))))
    {
