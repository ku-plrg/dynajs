...
export function AO__SerializeJSONProperty ($ : SpecRuntime, state : Wrapped<unknown>, key : Wrapped<string>, holder : Wrapped<unknown>) {
  ...
  if (($.isType(value, "object")) || ($.isType(value, "bigint")))
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 596, $.is(AO__IsCallable($, (toJSON as Wrapped<unknown>)), $.base<boolean>(true, []))))
    {
  ...
  if (!$.condition(Number.MAX_SAFE_INTEGER - 597, $.is(state["ReplacerFunction" /* TODO INTERNAL : internal access */], $.base<undefined>(undefined, []))))
  {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 598, $.is(value, $.base<null>(null, []))))
  {
    return $.base<string>("null", []);
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 599, $.is(value, $.base<boolean>(true, []))))
  {
    return $.base<string>("true", []);
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 600, $.is(value, $.base<boolean>(false, []))))
  {
  ...
  if (($.isType(value, "object")) && $.condition(Number.MAX_SAFE_INTEGER - 601, $.is(AO__IsCallable($, (value as Wrapped<unknown>)), $.base<boolean>(false, []))))
  {
    var isArray = AO__IsArray($, (value as Wrapped<unknown>));
    if ($.condition(Number.MAX_SAFE_INTEGER - 602, $.is(isArray, $.base<boolean>(true, []))))
    {
