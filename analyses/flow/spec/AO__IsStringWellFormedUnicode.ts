...
export function AO__IsStringWellFormedUnicode ($ : SpecRuntime, string : Lifted<string>) {
  ...
  while ($.value($.condition(Number.MAX_SAFE_INTEGER - 589, $.lessThan(k, len))))
  {
    var cp = AO__CodePointAt($, (string as Lifted<string>), (k as Lifted<number>));
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 590, $.is(cp["IsUnpairedSurrogate" /* TODO INTERNAL : internal access */], $.default<boolean>(true, [])))))
    {
