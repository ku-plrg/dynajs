...
export function AO__StringPad ($ : SpecRuntime, S : Lifted<string>, maxLength : Lifted<number>, fillString : Lifted<string>, placement : Lifted<unknown>) {
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 728, $.lessThanEqual(maxLength, stringLength))))
  {
    return S;
  }

  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 729, $.is(fillString, $.default<string>("", [])))))
  {
    return S;
  }
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 730, $.is(placement, $.default<string>("start", [])))))
  {
