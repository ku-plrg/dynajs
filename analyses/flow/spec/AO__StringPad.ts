...
export function AO__StringPad ($ : SpecRuntime, S : Lifted<string>, maxLength : Lifted<number>, fillString : Lifted<string>, placement : Lifted<unknown>) {
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 776, $.lessThanEqual(maxLength, stringLength))))
  {
    return S;
  }

  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 777, $.is(fillString, $.default<string>("", [])))))
  {
    return S;
  }
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 778, $.is(placement, $.default<string>("start", [])))))
  {
