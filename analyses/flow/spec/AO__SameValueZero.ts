...
export function AO__SameValueZero ($ : SpecRuntime, x : Lifted<unknown>, y : Lifted<unknown>) {
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 726, $.is(AO__SameType($, (x as Lifted<unknown>), (y as Lifted<unknown>)), $.default<boolean>(false, [])))))
  {
    return $.default<boolean>(false, []);
  }

  if (($.value($.condition(Number.MAX_SAFE_INTEGER - 727, $.isType(x, "number")))))
  {
