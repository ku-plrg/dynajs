...
export function INTRINSICS_String_prototype_replaceAll ($ : SpecRuntime, $this : Lifted<unknown>, searchValue : Lifted<unknown>, replaceValue : Lifted<unknown>) {
  ...
  if (!($.value($.condition(Number.MAX_SAFE_INTEGER - 461, $.is(searchValue, $.default<undefined>(undefined, [])))) || $.value($.condition(Number.MAX_SAFE_INTEGER - 462, $.is(searchValue, $.default<null>(null, []))))))
  {
    var isRegExp = AO__IsRegExp($, (searchValue as Lifted<unknown>));
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 463, $.is(isRegExp, $.default<boolean>(true, [])))))
    {
      var flags = AO__Get($, (searchValue as Lifted<unknown>), ($.default<string>("flags", []) as Lifted<unknown>));
      AO__RequireObjectCoercible($, flags);
      if (!$.value($.condition(Number.MAX_SAFE_INTEGER - 464, $.contains(AO__ToString($, (flags as Lifted<unknown>)), $.default<string>("g", [])))))
      {
        throw new TypeError;
      }
    ...
    if (!$.value($.condition(Number.MAX_SAFE_INTEGER - 465, $.is(replacer, $.default<undefined>(undefined, [])))))
    {
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 466, $.is(functionalReplace, $.default<boolean>(false, [])))))
  {
  ...
  while (!$.value($.condition(Number.MAX_SAFE_INTEGER - 467, $.is(position, $.default<string>("not-found", [])))))
  {
  ...
  for (var p of matchPositions)
  {
    ...
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 468, $.is(functionalReplace, $.default<boolean>(true, [])))))
    {
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 469, $.lessThan(endOfLastMatch, $.length(string)))))
  {
