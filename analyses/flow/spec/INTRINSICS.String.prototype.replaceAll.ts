...
export function INTRINSICS_String_prototype_replaceAll ($ : SpecRuntime, $this : Wrapped<unknown>, searchValue : Wrapped<unknown>, replaceValue : Wrapped<unknown>) {
  ...
  if (!($.condition(Number.MAX_SAFE_INTEGER - 405, $.is(searchValue, $.base<undefined>(undefined, []))) || $.condition(Number.MAX_SAFE_INTEGER - 406, $.is(searchValue, $.base<null>(null, [])))))
  {
    var isRegExp = AO__IsRegExp($, (searchValue as Wrapped<unknown>));
    if ($.condition(Number.MAX_SAFE_INTEGER - 407, $.is(isRegExp, $.base<boolean>(true, []))))
    {
    ...
    if (!$.condition(Number.MAX_SAFE_INTEGER - 408, $.is(replacer, $.base<undefined>(undefined, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 409, $.is(functionalReplace, $.base<boolean>(false, []))))
  {
  ...
  while (!$.condition(Number.MAX_SAFE_INTEGER - 410, $.is(position, $.base<string>("not-found", []))))
  {
  ...
  for (var p of matchPositions)
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 411, $.is(functionalReplace, $.base<boolean>(true, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 412, $.lessThan(endOfLastMatch, $.length(string))))
  {
