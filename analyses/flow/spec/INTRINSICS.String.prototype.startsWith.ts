...
export function INTRINSICS_String_prototype_startsWith ($ : SpecRuntime, $this : Lifted<unknown>, searchString : Lifted<unknown>, position : Lifted<unknown> = $.default<undefined>(undefined, [])) {
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 486, $.is(isRegExp, $.default<boolean>(true, [])))))
  {
    throw new TypeError;
  }
  ...
  var len = $.length(S);
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 487, $.is(position, $.default<undefined>(undefined, [])))))
  {
  ...
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 488, $.is(searchLength, $.default<number>(0, [])))))
  {
    return $.default<boolean>(true, []);
  }

  var end = $.add((start as Lifted<number>), (searchLength as Lifted<number>));
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 489, $.greaterThan(end, len))))
  {
    return $.default<boolean>(false, []);
  }

  var substring = $.substring(S, (start as Lifted<number>), (end as Lifted<number>));
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 490, $.is(substring, searchStr))))
  {
    return $.default<boolean>(true, []);
  }
