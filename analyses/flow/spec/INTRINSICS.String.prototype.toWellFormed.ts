...
export function INTRINSICS_String_prototype_toWellFormed ($ : SpecRuntime, $this : Wrapped<unknown>) {
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 438, $.lessThan(k, strLen)))
  {
    var cp = AO__CodePointAt($, (S as Wrapped<string>), (k as Wrapped<number>));
    if ($.condition(Number.MAX_SAFE_INTEGER - 439, $.is(cp["IsUnpairedSurrogate" /* TODO INTERNAL : internal access */], $.base<boolean>(true, []))))
    {
