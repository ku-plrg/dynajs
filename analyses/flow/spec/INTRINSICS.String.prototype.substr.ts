...
export function INTRINSICS_String_prototype_substr ($ : SpecRuntime, $this : Wrapped<unknown>, start : Wrapped<unknown>, length : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 465, $.is(intStart, $.lit<number>(-Infinity))))
  {
    intStart = $.lit<number>(0);
  }
  else
  {
    if ($.condition(Number.MAX_SAFE_INTEGER - 466, $.lessThan(intStart, $.lit<number>(0))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 467, $.is(length, $.lit<undefined>(undefined))))
  {
    var intLength = size;
  }
  else
