...
export function INTRINSICS_JSON_stringify ($ : SpecRuntime, $this : Lifted<unknown>, value : Lifted<unknown>, replacer : Lifted<unknown> = $.default<undefined>(undefined, []), space : Lifted<unknown> = $.default<undefined>(undefined, [])) {
  ...
  if (($.value($.condition(Number.MAX_SAFE_INTEGER - 258, $.isType(replacer, "object")))))
  {
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 259, $.is(AO__IsCallable($, (replacer as Lifted<unknown>)), $.default<boolean>(true, [])))))
    {
    ...
    else
    {
      ...
      if ($.value($.condition(Number.MAX_SAFE_INTEGER - 260, $.is(isArray, $.default<boolean>(true, [])))))
      {
        PropertyList = [] as Lifted<never>[];
        var len = AO__LengthOfArrayLike($, (replacer as Lifted<unknown>));
        var k = $.default<number>(0, []);
        while ($.value($.condition(Number.MAX_SAFE_INTEGER - 261, $.lessThan(k, len))))
        {
          ...
          if (($.value($.condition(Number.MAX_SAFE_INTEGER - 262, $.isType(v, "string")))))
          {
            item = v;
          }
          else
          {
            if (($.value($.condition(Number.MAX_SAFE_INTEGER - 263, $.isType(v, "number")))))
            {
              item = AO__ToString($, (v as Lifted<unknown>));
            }
            else
            {
              if (($.value($.condition(Number.MAX_SAFE_INTEGER - 264, $.isType(v, "object")))))
              {
          ...
          if (!$.value($.condition(Number.MAX_SAFE_INTEGER - 265, $.is(item, $.default<undefined>(undefined, [])))) && !$.value($.condition(Number.MAX_SAFE_INTEGER - 266, $.contains(PropertyList, item))))
          {
  ...
  if (($.value($.condition(Number.MAX_SAFE_INTEGER - 267, $.isType(space, "object")))))
  {
  ...
  if (($.value($.condition(Number.MAX_SAFE_INTEGER - 268, $.isType(space, "number")))))
  {
    var spaceMV = AO__ToIntegerOrInfinity($, (space as Lifted<unknown>));
    spaceMV = $.min($.default<number>(10, []), spaceMV);
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 269, $.lessThan(spaceMV, $.default<number>(1, [])))))
    {
  ...
  {
    if (($.value($.condition(Number.MAX_SAFE_INTEGER - 270, $.isType(space, "string")))))
    {
      if ($.value($.condition(Number.MAX_SAFE_INTEGER - 271, $.lessThanEqual($.length(space), $.default<number>(10, [])))))
      {
        var gap = space;
      }
      else
