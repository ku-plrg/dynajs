...
export function INTRINSICS_String_prototype_split ($ : SpecRuntime, $this : Wrapped<unknown>, separator : Wrapped<unknown>, limit : Wrapped<unknown>) {
  ...
  if (!($.condition(Number.MAX_SAFE_INTEGER - 401, $.is(separator, $.base<undefined>(undefined, []))) || $.condition(Number.MAX_SAFE_INTEGER - 402, $.is(separator, $.base<null>(null, [])))))
  {
    var splitter = AO__GetMethod($, (separator as Wrapped<unknown>), ($.base<symbol>(Symbol.split, []) as Wrapped<unknown>));
    if (!$.condition(Number.MAX_SAFE_INTEGER - 403, $.is(splitter, $.base<undefined>(undefined, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 404, $.is(limit, $.base<undefined>(undefined, []))))
  {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 405, $.is(lim, $.base<number>(0, []))))
  {
    return AO__CreateArrayFromList($, ([] as Wrapped<unknown>[]));
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 406, $.is(separator, $.base<undefined>(undefined, []))))
  {
    return AO__CreateArrayFromList($, ([S] as Wrapped<unknown>[]));
  }

  var separatorLength = $.length(R);
  if ($.condition(Number.MAX_SAFE_INTEGER - 407, $.is(separatorLength, $.base<number>(0, []))))
  {
    var strLen = $.length(S);
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 408, $.is(S, $.base<string>("", []))))
  {
    return AO__CreateArrayFromList($, ([S] as Wrapped<unknown>[]));
  }
  ...
  while (!$.condition(Number.MAX_SAFE_INTEGER - 409, $.is(j, $.base<string>("not-found", []))))
  {
    var T = $.substring(S, (i as Wrapped<number>), (j as Wrapped<number>));
    $.append(substrings, T)
    if ($.condition(Number.MAX_SAFE_INTEGER - 410, $.is($.base<number>(substrings.length, []), lim)))
    {
