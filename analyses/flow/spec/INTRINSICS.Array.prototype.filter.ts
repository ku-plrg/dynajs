...
export function INTRINSICS_Array_prototype_filter ($ : SpecRuntime, $this : Wrapped<unknown>, callback : Wrapped<unknown>, thisArg : Wrapped<unknown> = $.undef) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 126, $.is(AO__IsCallable($, (callback as Wrapped<unknown>)), $.base<boolean>(false, []))))
  {
    throw new TypeError;
  }
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 127, $.lessThan(k, len)))
  {
    var Pk = AO__ToString($, (k as Wrapped<unknown>));
    var kPresent = AO__HasProperty($, (O as Wrapped<unknown>), (Pk as Wrapped<unknown>));
    if ($.condition(Number.MAX_SAFE_INTEGER - 128, $.is(kPresent, $.base<boolean>(true, []))))
    {
      ...
      if ($.condition(Number.MAX_SAFE_INTEGER - 129, $.is(selected, $.base<boolean>(true, []))))
      {
