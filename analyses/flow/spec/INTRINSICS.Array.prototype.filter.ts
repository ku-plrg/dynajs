...
export function INTRINSICS_Array_prototype_filter ($ : SpecRuntime, $this : Lifted<unknown>, callback : Lifted<unknown>, thisArg : Lifted<unknown> = $.undef) {
  var O = AO__ToObject($, $this);
  var len = AO__LengthOfArrayLike($, (O as Lifted<unknown>));
  if ($.condition(Number.MAX_SAFE_INTEGER - 136, $.is(AO__IsCallable($, (callback as Lifted<unknown>)), $.lit<boolean>(false))))
  {
    throw new TypeError;
  }
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 137, $.lessThan(k, len)))
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 138, $.is(kPresent, $.lit<boolean>(true))))
    {
      ...
      if ($.condition(Number.MAX_SAFE_INTEGER - 139, $.is(selected, $.lit<boolean>(true))))
      {
        ...
        to = $.add((to as Lifted<number>), ($.lit<number>(1) as Lifted<number>));
      }
