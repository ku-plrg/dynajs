...
export function AO__GetSubstitution ($ : SpecRuntime, matched : Wrapped<string>, str : Wrapped<string>, position : Wrapped<number>, captures : Wrapped<string | undefined>[], namedCaptures : Wrapped<unknown>, replacementTemplate : Wrapped<string>) {
  ...
  while (!$.condition(Number.MAX_SAFE_INTEGER - 72, $.is(templateRemainder, $.base<string>("", []))))
  {
        ...
        else
        {
          ...
          else
          {
            if ($.condition(Number.MAX_SAFE_INTEGER - 77, $.is($.substring(templateRemainder, $.base(0, []), $.base(1, [])), $.base("$", [])) && /[0-9]/.test($.peek($.substring(templateRemainder, $.base(1, []), $.base(2, []))))))
            {
              ...
              if ($.condition(Number.MAX_SAFE_INTEGER - 78, $.greaterThan(index, captureLen)) && $.condition(Number.MAX_SAFE_INTEGER - 79, $.is(digitCount, $.base<number>(2, []))))
              {
              ...
              if (($.condition(Number.MAX_SAFE_INTEGER - 80, $.greaterThanEqual(index, $.base<number>(1, []))) && $.condition(Number.MAX_SAFE_INTEGER - 81, $.lessThanEqual(index, captureLen))))
              {
                var capture = captures[$.subtract((index as Wrapped<number>), ($.base<number>(1, []) as Wrapped<number>))];
                if ($.condition(Number.MAX_SAFE_INTEGER - 82, $.is(capture, $.base<undefined>(undefined, []))))
                {
                  var refReplacement = $.base<string>("", []);
                }
                else
            ...
            else
            {
              if ($.condition(Number.MAX_SAFE_INTEGER - 83, $.is($.substring(templateRemainder, $.base(0, []), $.base(2, [])), $.base("$<", []))))
              {
                ...
                if ($.condition(Number.MAX_SAFE_INTEGER - 84, $.is(gtPos, $.base<string>("not-found", []))) || $.condition(Number.MAX_SAFE_INTEGER - 85, $.is(namedCaptures, $.base<undefined>(undefined, []))))
                {
                  var ref = $.base<string>("$<", []);
                  var refReplacement = ref;
                }
                else
                {
                  ...
                  if ($.condition(Number.MAX_SAFE_INTEGER - 86, $.is(capture, $.base<undefined>(undefined, []))))
                  {
                    var refReplacement = $.base<string>("", []);
                  }
                  else
