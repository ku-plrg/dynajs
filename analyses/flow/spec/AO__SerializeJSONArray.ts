...
export function AO__SerializeJSONArray ($ : SpecRuntime, state : Lifted<unknown>, value : Lifted<unknown>) {
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 728, $.contains(state["Stack" /* TODO INTERNAL : internal access */], value))))
  {
  ...
  while ($.value($.condition(Number.MAX_SAFE_INTEGER - 729, $.lessThan(index, len))))
  {
    ...
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 730, $.is(strP, $.default<undefined>(undefined, [])))))
    {
  ...
  {
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 731, $.is(state["Gap" /* TODO INTERNAL : internal access */], $.default<string>("", [])))))
    {
