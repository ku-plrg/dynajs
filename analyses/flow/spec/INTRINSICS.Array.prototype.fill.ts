...
export function INTRINSICS_Array_prototype_fill ($ : SpecRuntime, $this : Wrapped<unknown>, value : Wrapped<unknown>, start : Wrapped<unknown> = $.undef, end : Wrapped<unknown> = $.undef) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 120, $.is(relativeStart, $.base<number>(-Infinity, []))))
  {
    var k = $.base<number>(0, []);
  }
  else
  {
    if ($.condition(Number.MAX_SAFE_INTEGER - 121, $.lessThan(relativeStart, $.base<number>(0, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 122, $.is(end, $.base<undefined>(undefined, []))))
  {
    var relativeEnd = len;
  }
  else
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 123, $.is(relativeEnd, $.base<number>(-Infinity, []))))
  {
    var final = $.base<number>(0, []);
  }
  else
  {
    if ($.condition(Number.MAX_SAFE_INTEGER - 124, $.lessThan(relativeEnd, $.base<number>(0, []))))
    {
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 125, $.lessThan(k, final)))
  {
