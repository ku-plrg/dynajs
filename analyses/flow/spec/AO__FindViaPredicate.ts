...
export function AO__FindViaPredicate ($ : SpecRuntime, O : Wrapped<unknown>, len : Wrapped<number>, direction : Wrapped<unknown>, predicate : Wrapped<unknown>, thisArg : Wrapped<unknown>) {
  if ($.condition(Number.MAX_SAFE_INTEGER - 37, $.is(AO__IsCallable($, (predicate as Wrapped<unknown>)), $.base<boolean>(false, []))))
  {
    throw new TypeError;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 38, $.is(direction, $.base<string>("ascending", []))))
  {
    var indices = $.range($.base<number>(0, []), true, len, false, true, Number.MAX_SAFE_INTEGER - 39);
  }
  else
  {
    var indices = $.range($.base<number>(0, []), true, len, false, false, Number.MAX_SAFE_INTEGER - 40);
  }
  ...
  for (var k of indices)
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 41, $.is(AO__ToBoolean($, (testResult as Wrapped<unknown>)), $.base<boolean>(true, []))))
    {
