...
export function AO__FindViaPredicate ($ : SpecRuntime, O : Wrapped<unknown>, len : Wrapped<number>, direction : Wrapped<unknown>, predicate : Wrapped<unknown>, thisArg : Wrapped<unknown>) {
  if ($.condition(Number.MAX_SAFE_INTEGER - 47, $.is(AO__IsCallable($, (predicate as Wrapped<unknown>)), $.lit<boolean>(false))))
  {
    throw new TypeError;
  }
  ...
  for (var k of indices)
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 51, $.is(AO__ToBoolean($, (testResult as Wrapped<unknown>)), $.lit<boolean>(true))))
    {
