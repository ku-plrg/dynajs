...
export function INTRINSICS_String_prototype_replace ($ : SpecRuntime, $this : Wrapped<unknown>, searchValue : Wrapped<unknown>, replaceValue : Wrapped<unknown>) {
  ...
  if (!($.condition(Number.MAX_SAFE_INTEGER - 378, $.is(searchValue, $.base<undefined>(undefined, []))) || $.condition(Number.MAX_SAFE_INTEGER - 379, $.is(searchValue, $.base<null>(null, [])))))
  {
    var replacer = AO__GetMethod($, (searchValue as Wrapped<unknown>), ($.base<symbol>(Symbol.replace, []) as Wrapped<unknown>));
    if (!$.condition(Number.MAX_SAFE_INTEGER - 380, $.is(replacer, $.base<undefined>(undefined, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 381, $.is(functionalReplace, $.base<boolean>(false, []))))
  {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 382, $.is(position, $.base<string>("not-found", []))))
  {
    return string;
  }
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 383, $.is(functionalReplace, $.base<boolean>(true, []))))
  {
