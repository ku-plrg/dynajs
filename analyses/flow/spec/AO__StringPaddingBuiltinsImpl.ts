...
export function AO__StringPaddingBuiltinsImpl ($ : SpecRuntime, O : Wrapped<unknown>, maxLength : Wrapped<unknown>, fillString : Wrapped<unknown>, placement : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 619, $.lessThanEqual(intMaxLength, stringLength)))
  {
    return S;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 620, $.is(fillString, $.base<undefined>(undefined, []))))
  {
