...
export function AO__StringPaddingBuiltinsImpl ($ : SpecRuntime, O : Wrapped<unknown>, maxLength : Wrapped<unknown>, fillString : Wrapped<unknown>, placement : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 623, $.lessThanEqual(intMaxLength, stringLength)))
  {
    return S;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 624, $.is(fillString, $.base<undefined>(undefined, []))))
  {
