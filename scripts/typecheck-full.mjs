...
function findNoCheck(dir, out = []) {
  for (const e of readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (e.name.endsWith(".ts") && NOCHECK_RE.test(readFileSync(p, "utf8"))) out.push(p);
  }
  return out;
}
...
if (failed) {
  console.log(chalk.yellow("✗ errors above are suppressed by // @ts-nocheck during a normal build"));
  process.exit(1);
}
