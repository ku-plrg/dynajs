...
export function INTRINSICS_Array_prototype_every ($ : SpecRuntime, $this : Wrapped<unknown>, callback : Wrapped<unknown>, thisArg : Wrapped<unknown> = $.undef) {
  var O = AO__ToObject($, $this);
  var len = AO__LengthOfArrayLike($, (O as Wrapped<unknown>));
  if ($.condition(Number.MAX_SAFE_INTEGER - 99, $.is(AO__IsCallable($, (callback as Wrapped<unknown>)), $.base<boolean>(false, []))))
  {
    throw new TypeError;
  }

  var k = $.base<number>(0, []);
  while ($.condition(Number.MAX_SAFE_INTEGER - 100, $.lessThan(k, len)))
  {
    var Pk = AO__ToString($, (k as Wrapped<unknown>));
    var kPresent = AO__HasProperty($, (O as Wrapped<unknown>), (Pk as Wrapped<unknown>));
    if ($.condition(Number.MAX_SAFE_INTEGER - 101, $.is(kPresent, $.base<boolean>(true, []))))
    {
      ...
      if ($.condition(Number.MAX_SAFE_INTEGER - 102, $.is(testResult, $.base<boolean>(false, []))))
      {
