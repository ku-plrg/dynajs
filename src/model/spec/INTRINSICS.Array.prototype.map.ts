...
export function INTRINSICS_Array_prototype_map ($ : SpecRuntime, $this : Wrapped<unknown>, callback : Wrapped<unknown>, thisArg : Wrapped<unknown> = $.undef) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 145, $.is(AO__IsCallable($, (callback as Wrapped<unknown>)), $.base<boolean>(false, []))))
  {
    throw new TypeError;
  }
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 146, $.lessThan(k, len)))
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 147, $.is(kPresent, $.base<boolean>(true, []))))
    {
