...
export function INTRINSICS_Array_prototype_join ($ : SpecRuntime, $this : Wrapped<unknown>, separator : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 145, $.is(separator, $.base<undefined>(undefined, []))))
  {
    var sep = $.base<string>(",", []);
  }
  else
  ...
  var R = $.base<string>("", []);
  var k = $.base<number>(0, []);
  while ($.condition(Number.MAX_SAFE_INTEGER - 146, $.lessThan(k, len)))
  {
    if ($.condition(Number.MAX_SAFE_INTEGER - 147, $.greaterThan(k, $.base<number>(0, []))))
    {
      R = $.concatenate(R, sep);
    }

    var element = AO__Get($, (O as Wrapped<unknown>), (AO__ToString($, (k as Wrapped<unknown>)) as Wrapped<unknown>));
    if (!($.condition(Number.MAX_SAFE_INTEGER - 148, $.is(element, $.base<undefined>(undefined, []))) || $.condition(Number.MAX_SAFE_INTEGER - 149, $.is(element, $.base<null>(null, [])))))
    {
