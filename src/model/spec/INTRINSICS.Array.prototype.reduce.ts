...
export function INTRINSICS_Array_prototype_reduce ($ : SpecRuntime, $this : Wrapped<unknown>, callback : Wrapped<unknown>, initialValue : Wrapped<unknown> = $.undef) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 161, $.is(AO__IsCallable($, (callback as Wrapped<unknown>)), $.base<boolean>(false, []))))
  {
    throw new TypeError;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 162, $.is(len, $.base<number>(0, []))) && !initialValueIsPresent)
  {
    throw new TypeError;
  }
  ...
  else
  {
    ...
    while ($.condition(Number.MAX_SAFE_INTEGER - 163, $.is(kPresent, $.base<boolean>(false, []))) && $.condition(Number.MAX_SAFE_INTEGER - 164, $.lessThan(k, len)))
    {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 166, $.is(kPresent, $.base<boolean>(false, []))))
    {
      throw new TypeError;
    }
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 167, $.lessThan(k, len)))
  {
    var Pk = AO__ToString($, (k as Wrapped<unknown>));
