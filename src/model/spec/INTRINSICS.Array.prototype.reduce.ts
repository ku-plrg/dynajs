...
export function INTRINSICS_Array_prototype_reduce ($ : SpecRuntime, $this : Wrapped<unknown>, callback : Wrapped<unknown>, initialValue : Wrapped<unknown> = $.undef) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 157, $.is(len, $.base<number>(0, []))) && !initialValueIsPresent)
  {
    throw new TypeError;
  }
  ...
  else
  {
    ...
    while ($.condition(Number.MAX_SAFE_INTEGER - 158, $.is(kPresent, $.base<boolean>(false, []))) && $.condition(Number.MAX_SAFE_INTEGER - 159, $.lessThan(k, len)))
    {
      var Pk = AO__ToString($, (k as Wrapped<unknown>));
      kPresent = AO__HasProperty($, (O as Wrapped<unknown>), (Pk as Wrapped<unknown>));
      if ($.condition(Number.MAX_SAFE_INTEGER - 160, $.is(kPresent, $.base<boolean>(true, []))))
      {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 161, $.is(kPresent, $.base<boolean>(false, []))))
    {
      throw new TypeError;
    }
  ...
  while ($.condition(Number.MAX_SAFE_INTEGER - 162, $.lessThan(k, len)))
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 163, $.is(kPresent, $.base<boolean>(true, []))))
    {
