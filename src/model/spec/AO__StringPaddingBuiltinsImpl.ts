...
export function AO__StringPaddingBuiltinsImpl ($ : SpecRuntime, O : Wrapped<unknown>, maxLength : Wrapped<unknown>, fillString : Wrapped<unknown>, placement : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 626, $.lessThanEqual(intMaxLength, stringLength)))
  {
    return S;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 627, $.is(fillString, $.base<undefined>(undefined, []))))
  {
