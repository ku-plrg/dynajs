...
export function AO__StringPad ($ : SpecRuntime, S : Wrapped<string>, maxLength : Wrapped<number>, fillString : Wrapped<string>, placement : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 567, $.lessThanEqual(maxLength, stringLength)))
  {
    return S;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 568, $.is(fillString, $.base<string>("", []))))
  {
    return S;
  }
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 569, $.is(placement, $.base<string>("start", []))))
  {
