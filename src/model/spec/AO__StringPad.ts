...
export function AO__StringPad ($ : SpecRuntime, S : Wrapped<string>, maxLength : Wrapped<number>, fillString : Wrapped<string>, placement : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 623, $.lessThanEqual(maxLength, stringLength)))
  {
    return S;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 624, $.is(fillString, $.base<string>("", []))))
  {
    return S;
  }
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 625, $.is(placement, $.base<string>("start", []))))
  {
