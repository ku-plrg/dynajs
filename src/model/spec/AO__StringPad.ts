...
export function AO__StringPad ($ : SpecRuntime, S : Wrapped<string>, maxLength : Wrapped<number>, fillString : Wrapped<string>, placement : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 593, $.lessThanEqual(maxLength, stringLength)))
  {
    return S;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 594, $.is(fillString, $.base<string>("", []))))
  {
    return S;
  }
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 595, $.is(placement, $.base<string>("start", []))))
  {
