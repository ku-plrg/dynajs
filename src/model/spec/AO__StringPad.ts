...
export function AO__StringPad ($ : SpecRuntime, S : Wrapped<string>, maxLength : Wrapped<number>, fillString : Wrapped<string>, placement : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 595, $.lessThanEqual(maxLength, stringLength)))
  {
    return S;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 596, $.is(fillString, $.base<string>("", []))))
  {
    return S;
  }
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 597, $.is(placement, $.base<string>("start", []))))
  {
