...
export function INTRINSICS_String_prototype_replaceAll ($ : SpecRuntime, $this : Wrapped<unknown>, searchValue : Wrapped<unknown>, replaceValue : Wrapped<unknown>) {
  ...
  if (!($.condition(Number.MAX_SAFE_INTEGER - 382, $.is(searchValue, $.base<undefined>(undefined, []))) || $.condition(Number.MAX_SAFE_INTEGER - 383, $.is(searchValue, $.base<null>(null, [])))))
  {
    var isRegExp = AO__IsRegExp($, (searchValue as Wrapped<unknown>));
    if ($.condition(Number.MAX_SAFE_INTEGER - 384, $.is(isRegExp, $.base<boolean>(true, []))))
    {
    ...
    if (!$.condition(Number.MAX_SAFE_INTEGER - 385, $.is(replacer, $.base<undefined>(undefined, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 386, $.is(functionalReplace, $.base<boolean>(false, []))))
  {
  ...
  while (!$.condition(Number.MAX_SAFE_INTEGER - 387, $.is(position, $.base<string>("not-found", []))))
  {
  ...
  for (var p of matchPositions)
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 388, $.is(functionalReplace, $.base<boolean>(true, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 389, $.lessThan(endOfLastMatch, $.length(string))))
  {
