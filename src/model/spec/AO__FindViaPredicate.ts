...
export function AO__FindViaPredicate ($ : SpecRuntime, O : Wrapped<unknown>, len : Wrapped<number>, direction : Wrapped<unknown>, predicate : Wrapped<unknown>, thisArg : Wrapped<unknown>) {
  if ($.condition(Number.MAX_SAFE_INTEGER - 31, $.is(AO__IsCallable($, (predicate as Wrapped<unknown>)), $.base<boolean>(false, []))))
  {
    throw new TypeError;
  }

  if ($.condition(Number.MAX_SAFE_INTEGER - 32, $.is(direction, $.base<string>("ascending", []))))
  {
  ...
  for (var _x0 = 0; _x0 < indices.length; _x0++)
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 33, $.is(AO__ToBoolean($, (testResult as Wrapped<unknown>)), $.base<boolean>(true, []))))
    {
