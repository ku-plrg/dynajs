...
export function INTRINSICS_String_prototype_at ($ : SpecRuntime, $this : Wrapped<unknown>, index : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 340, $.greaterThanEqual(relativeIndex, $.base<number>(0, []))))
  {
    var k = relativeIndex;
  }
  else
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 341, $.lessThan(k, $.base<number>(0, []))) || $.condition(Number.MAX_SAFE_INTEGER - 342, $.greaterThanEqual(k, len)))
  {
