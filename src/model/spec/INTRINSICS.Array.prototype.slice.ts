...
export function INTRINSICS_Array_prototype_slice ($ : SpecRuntime, $this : Wrapped<unknown>, start : Wrapped<unknown>, end : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 184, $.is(relativeStart, $.base<number>(-Infinity, []))))
  {
    var k = $.base<number>(0, []);
  }
  else
  {
    if ($.condition(Number.MAX_SAFE_INTEGER - 185, $.lessThan(relativeStart, $.base<number>(0, []))))
    {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 186, $.is(end, $.base<undefined>(undefined, []))))
  {
    var relativeEnd = len;
  }
  else
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 187, $.is(relativeEnd, $.base<number>(-Infinity, []))))
  {
    var final = $.base<number>(0, []);
  }
  else
  {
    if ($.condition(Number.MAX_SAFE_INTEGER - 188, $.lessThan(relativeEnd, $.base<number>(0, []))))
    {
  ...
  var n = $.base<number>(0, []);
  while ($.condition(Number.MAX_SAFE_INTEGER - 189, $.lessThan(k, final)))
  {
    var Pk = AO__ToString($, (k as Wrapped<unknown>));
    var kPresent = AO__HasProperty($, (O as Wrapped<unknown>), (Pk as Wrapped<unknown>));
    if ($.condition(Number.MAX_SAFE_INTEGER - 190, $.is(kPresent, $.base<boolean>(true, []))))
    {
