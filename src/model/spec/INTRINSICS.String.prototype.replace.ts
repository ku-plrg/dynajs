...
export function INTRINSICS_String_prototype_replace ($ : BootStrap, $this : Wrapped<unknown>, searchValue : Wrapped<unknown>, replaceValue : Wrapped<unknown>) {
  ...
  if ($.is(position, $.base<string>("not-found", [])))
  {
    return string;
  }

  var preceding = $.substring(string, $.base<number>(0, []), position);
