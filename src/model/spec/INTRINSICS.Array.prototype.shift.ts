...
export function INTRINSICS_Array_prototype_shift ($ : SpecRuntime, $this : Wrapped<unknown>) {
  ...
  if ($.condition(Number.MAX_SAFE_INTEGER - 186, $.is(len, $.base<number>(0, []))))
  {
  ...
  var k = $.base<number>(1, []);
  while ($.condition(Number.MAX_SAFE_INTEGER - 187, $.lessThan(k, len)))
  {
    ...
    if ($.condition(Number.MAX_SAFE_INTEGER - 188, $.is(fromPresent, $.base<boolean>(true, []))))
    {
