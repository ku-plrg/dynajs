...
export function INTRINSICS_Array_prototype_every ($ : LiftedTransferOps, $this : Lifted<unknown>, callback : Lifted<unknown>, thisArg : Lifted<unknown> = $.default<undefined>(undefined, [])) {
  var O = AO__ToObject($, $this);
  var len = AO__LengthOfArrayLike($, (O as Lifted<unknown>));
  if ($.value($.condition(Number.MAX_SAFE_INTEGER - 135, $.is(AO__IsCallable($, (callback as Lifted<unknown>)), $.default<boolean>(false, [])))))
  {
    throw new TypeError;
  }
  ...
  while ($.value($.condition(Number.MAX_SAFE_INTEGER - 136, $.lessThan(k, len))))
  {
    ...
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 137, $.is(kPresent, $.default<boolean>(true, [])))))
    {
      ...
      if ($.value($.condition(Number.MAX_SAFE_INTEGER - 138, $.is(testResult, $.default<boolean>(false, [])))))
      {
