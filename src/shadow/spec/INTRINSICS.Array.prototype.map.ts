...
export function INTRINSICS_Array_prototype_map ($ : LiftedTransferOps, $this : Lifted<unknown>, callback : Lifted<unknown>, thisArg : Lifted<unknown> = $.default<undefined>(undefined, [])) {
  ...
  while ($.value($.condition(Number.MAX_SAFE_INTEGER - 182, $.lessThan(k, len))))
  {
    ...
    if ($.value($.condition(Number.MAX_SAFE_INTEGER - 183, $.is(kPresent, $.default<boolean>(true, [])))))
    {
