...
export function tryToRegisterWarningHook(): void {
  ...
  import('node:module').then(({ registerHooks }) => {
    ...
    const target = new Set(['vm', 'node:vm', 'vm/promises', 'node:vm/promises']);
    ...
    warn('Failed to register warning hook. Code loaded by `vm` modules will not be instrumented. It is your responsibility to bridge them with dynajs API exposed via `globalThis.D$.instrument` to ensure complete analysis results.');
  });
}